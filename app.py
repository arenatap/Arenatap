...
# ─── Serve Frontend (GitHub Pages fallback) ────────────────────────────────────
...
def serve_static(filename):
    """Serve static files (images, etc.) from the app directory."""
    import mimetypes
    base_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    if os.path.isfile(filepath):
        ...
        return send_from_directory(base_dir, filename, mimetype=mime)
    return "Not found", 404
...
def new_season():
    try:
        ...
        # ── Auto-save current season results before resetting ──
        try:
            active_seasons = sb_get("seasons", {"is_active":"eq.true", "select":"id"})
            if active_seasons:
                old_season_id = active_seasons[0]["id"]

                # Get all users with season_coins > 0, ranked
                users = sb_get("users", {
                    "season_coins": "gt.0",
                ...
                if users:
                    ...
                    for rank, u in enumerate(users, 1):
                        _, err = sb_insert("season_results", {
                            "season_id": old_season_id,
                            "wallet_address": u["wallet_address"],
                            "username": u.get("username", ""),
                            "first_name": u.get("first_name", ""),
                            "final_coins": u["season_coins"],
                            "rank": rank
                        })
                        if not err:
                            saved_count += 1
                    logger.info(f"Season results saved: {saved_count}/{len(users)} users for season {old_season_id}")
        except Exception as e:
